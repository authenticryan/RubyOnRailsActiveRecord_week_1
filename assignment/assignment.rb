...
class Assignment
  ...
  def create_user(params)
      # accept a hash of user properties (`:username` and `:password_digest`) as an input parameter. Note these are 100% same as model class.i
  ...
  def update_listname(id, name)
      ...
      # (no return is required)
			...
			list_item = TodoList.find_by(id: id)
			list_item.list_name = "#{name}"
			list_item.save
  end 
